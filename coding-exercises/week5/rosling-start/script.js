...
function gotData(incomingData) {
  ...
  let rScale = d3.scaleLinear().domain(popExtent).range([10, 80]);
  ...
  function drawViz() {
    ...
    enteringElements.append("circle")
      ...
      .attr("fill", function (d) {
        return "url(#" + d.Country.toLowerCase().replace(/ /g, "-") + ")"
      })
      // .oppacity(0.9)
      ;
    ...
    defs.selectAll(".flag-pattern")
      ...
      .attr("id", function (d) {
        return d.Country.toLowerCase().replace(/ /g, "-")
      })
      .attr("height", "100%")
      .attr("width", "100%")
    ...
    enteringElements.transition().duration(50).attr("transform", getGroupLocation);
